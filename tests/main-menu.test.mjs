...
test('editor topbar reveals after its UI contributors and before heavy catalog boot', () => {
  ...
  const earlyReveal = bootstrap.indexOf('revealEditorChrome()', instructions)
  const catalog = bootstrap.indexOf("await import('./ldraw/catalog-v3.js')")
  const ready = bootstrap.indexOf('window.__bricklabRuntimeReady = true')
  const finalReveal = bootstrap.lastIndexOf('revealEditorChrome()')

  assert.ok(guard >= 0 && guard < appBoot)
  assert.ok(appBoot < overlay)
  assert.ok(overlay < instructions)
  assert.ok(instructions < earlyReveal)
  assert.ok(earlyReveal < catalog)
  assert.ok(catalog < ready)
  assert.ok(ready < finalReveal)
  assert.match(bootstrap, /setTimeout\?\.\(\(\) => revealEditorChrome\(\), 4500\)/)
...
test('v4 boot cache tag and responsive composition are current', () => {
  assert.match(index, /bootstrap\.js\?v=editor-topbar-20260914-v2/)
