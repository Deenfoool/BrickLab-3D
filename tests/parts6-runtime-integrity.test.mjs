...
test('PARTS-6 visual layers do not replace connector or mechanics metadata', async () => {
  ...
  assert.match(sources[8], /PARTS6_BENT_LIFTARM_FIDELITY_VERSION = 'parts-6-bent-liftarm-fidelity-v6'/)
  assert.match(sources[8], /rightAngleOutline/)
  assert.match(sources[8], /roundedOuterElbow/)
  assert.match(sources[9], /PARTS6_HERO_MICRO_DETAIL_VERSION = 'parts-6-hero-micro-detail-v5'/)
  assert.match(sources[9], /tyre-centre-mold-seam/)
  assert.match(sources[9], /gearbox-fill-plug/)
  assert.match(sources[9], /differential-spider-pin-retainer/)
  assert.match(sources[10], /PARTS6_CONNECTOR_FIDELITY_VERSION = 'parts-6-connector-fidelity-v3'/)
  assert.match(sources[10], /one visual owner per mating port/)
  assert.match(sources[11], /wrapPart/)
  assert.match(sources[12], /hardenFineDetailTree/)
  assert.match(sources[12], /hardenHeroMicroDetailTree/)
  assert.match(sources[13], /PARTS6_RACK_GEAR_FIDELITY_VERSION = 'parts-6-rack-gear-fidelity-v4'/)
  assert.match(sources[13], /LINEAR_PITCH = Math\.PI \* GEAR_MODULE_STUD/)
  assert.match(sources[13], /freezeLegacyBoundsCollider/)
})
