...
test('PARTS-5 is the final visual owner after legacy detail/material wrappers', async () => {
  const runtime = await readFile(new URL('runtime-extensions.js', root), 'utf8')
  ...
  const parts5 = runtime.indexOf("import('./parts5/visual-overhaul-v1.js?v=parts-5-20260909-visual-v1')")
  const physics = runtime.indexOf("import('./physics-v2.js')")
  assert.ok(legacyVisual >= 0 && legacyWheelMaterial > legacyVisual)
  assert.ok(parts5 > legacyWheelMaterial)
  assert.ok(physics > parts5)
})
...
test('visual QA gallery is checked into the repository', async () => {
