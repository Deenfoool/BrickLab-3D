...
const recovered71708Text = '0 Technic Beam 2 x 3 Liftarm Bent 90 Quarter Ellipse\n0 Name: 71708.dat\n0 !LDRAW_ORG Part\n1 16 0 0 0 1 0 0 0 1 0 0 0 1 axl3hole.dat\n'
const axleHoleShadowText = '0 !LDCAD SNAP_CYL [id=axle-hole] [gender=F] [caps=none] [secs=A 6 20] [center=true] [slide=true]\n'

globalThis.fetch = async input => {
  const url = String(input)
  if (url.includes('shadow-manifest.json')) {
    return new Response(JSON.stringify({ commit:SHADOW_SOURCE_V4.commit, files:['parts/qa-first-load.dat','p/axl3hole.dat'] }), {
      status:200,
      headers:{'content-type':'application/json'},
    })
  }
  if (url.includes('/mrkrstphr/ldraw-parts/main/parts/71708.dat')) return new Response(recovered71708Text, {status:200})
  if (url.includes('/pybricks/ldraw/master/parts/71708.dat')) return new Response('', {status:404})
  if (url.includes('/mrkrstphr/ldraw-parts/main/parts/qa-first-load.dat')) return new Response(officialText, {status:200})
  if (url.includes('raw.githubusercontent.com/pybricks/ldraw/')) return new Response(officialText, {status:200})
  if (url.includes(SHADOW_SOURCE_V4.repository) && url.endsWith('/parts/qa-first-load.dat')) return new Response(shadowText, {status:200})
  if (url.includes(SHADOW_SOURCE_V4.repository) && url.endsWith('/p/axl3hole.dat')) return new Response(axleHoleShadowText, {status:200})
...
test('connector hydration uses the visual LDraw mirror when pybricks returns 404 for 71708', async () => {
  const id = 'ldraw-71708-mirror-recovery'
  const def = {
    id,
    name:'71708 mirror recovery',
    connectors:[],
    ldraw:{file:'71708.dat',code:'71708',ready:true,level:'visual'},
    create() {
      const root = new THREE.Group()
      root.userData.partId = id
      const visual = new THREE.Group()
  ...
  const root = def.create()
  const connectivity = await v4.hydrate(def, root)

  assert.equal(connectivity?.status, 'ready')
  assert.ok(
    connectivity?.connectors?.some(connector => connector.gender === 'female'
      && connector.geometry?.sections?.some(section => section.shape === 'A' && section.radiusLdu === 6)),
    JSON.stringify(connectivity?.connectors),
  )
  assert.notEqual(connectivity?.warnings?.[0]?.code, 'hydrate-error')
