...
test('Search input, favorites, sections and keyboard isolation',async()=>{
  ...
  assert.equal(root.querySelectorAll('.pl-card').length,1)
  assert.match(root.querySelector('.pl-card .pl-select > small').textContent,/Axles \/ Standard/)
  input.value='3020';input.dispatchEvent(new window.Event('input',{bubbles:true}));await new Promise(r=>setTimeout(r,130))
  assert.match(root.querySelector('.pl-results').textContent,/Plate 2 x 4/,'exact design IDs search across family boundaries')
  input.value='ось 5';input.dispatchEvent(new window.Event('input',{bubbles:true}));await new Promise(r=>setTimeout(r,130))
...
test('Production catalog uses an isolated cache generation for atlas previews',async()=>{
  ...
  assert.match(map['./app.js'],/^\.\/app\.js\?v=/)
  assert.match(map['./ldraw/runtime-v3.js?v=ldraw-catalog-20260910-v3'],/runtime-metadata-cache-v1/)
  assert.match(bootstrap,/\.\/ldraw\/catalog-v3\.js\?v=ldraw-atlas-display-[^'" ]+/)
  ...
  assert.match(catalog,/\.\/library-atlas-v1\.js\?v=ldraw-atlas-[^'" ]+/)
