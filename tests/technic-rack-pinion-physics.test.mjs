...
test('SIMULATE rack-pinion runtime stays inside the authoritative physics microstep and preserves prototype owners', async () => {
  ...
  assert.match(source, /rack-guide-axis-mismatch/)
  assert.match(source, /MIN_GUIDE_ALIGNMENT = 0\.96/)
  ...
  assert.match(source, /detection-error:/)
  assert.match(source, /coupling-error:/)
