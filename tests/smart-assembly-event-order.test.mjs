...
test('Smart Assembly evaluates after catalog selection and uses a fresh production entrypoint', async () => {
  ...
  const activationImport = bootstrap.indexOf("./guidance/smart-assembly-activation-v1.js?v=smart-assembly-20260911-v2")
  ...
  const bootstrapUrls = index.match(/bootstrap\.js\?v=smart-assembly-20260911-v5/g) ?? []
  assert.equal(bootstrapUrls.length, 2, 'import map and module script must both use the fresh bootstrap generation')
  ...
  assert.match(activation, /smart-assembly-runtime-v1\.js\?v=smart-assembly-20260911-v4/, 'activation must request the fresh runtime generation')
  assert.match(runtime, /function inspectorSelectedObject\(\)/, 'guidance must have a live-inspector selection fallback')
  assert.match(runtime, /MutationObserver\(scheduleEvaluation\)/, 'guidance must react after the lexical editor updates selectedId')
})
