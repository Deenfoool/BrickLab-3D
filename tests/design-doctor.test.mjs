...
test('production Design Doctor mounts lazily and defaults to quiet non-invasive interaction', async () => {
  ...
  const doctorActivation = bootstrap.indexOf("./guidance/design-doctor-activation-v1.js?v=design-doctor-20260912-v3")
  ...
  assert.match(activation, /insertBefore\(button, deleteButton\)/)
  assert.match(runtime, /showIssue\(currentIndex \+ 1\)/, 'Next issue must not force camera focus')
  assert.match(runtime, /if \(issues\.length\) showIssue\(0\)/, 'first result is shown without auto-focus')
  assert.doesNotMatch(runtime, /document\.addEventListener\(['"]pointerup['"]/, 'ordinary pointer-up must not trigger rescans')
  assert.match(runtime, /setTimeout\(\(\) => void startScan\(\), 850\)/, 'automatic refresh is debounced after real model changes')
  assert.match(runtime, /quiet:true/)
  assert.match(runtime, /highlightIssue/)
  assert.doesNotMatch(runtime, /flashInspect|markProblem/, 'scan should not flash every inspected or problematic object')
