...
test('production bootstrap never blocks the established editor UI on Smart Assembly', async () => {
  ...
  const assistant = source.indexOf("./guidance/smart-assembly-activation-v1.js?v=smart-assembly-20260911-v2")
  assert.ok(adapter >= 0)
  ...
  assert.match(activation, /smart-assembly-runtime-v1\.js\?v=smart-assembly-20260911-v4/)
  ...
  assert.match(runtime, /function inspectorSelectedObject\(\)/, 'guidance must survive an unavailable group-selection capture')
  assert.match(runtime, /MutationObserver\(scheduleEvaluation\)/, 'guidance must observe the lexical inspector selection update')
