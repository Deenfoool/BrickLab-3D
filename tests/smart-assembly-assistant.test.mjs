...
test('production bootstrap starts Smart Assembly immediately after the editor contract without blocking later UI', async () => {
  ...
  const assistant = source.indexOf("./guidance/smart-assembly-activation-v1.js?v=smart-assembly-20260912-v1")
  const performance = source.indexOf("./performance/runtime-v1.js?v=performance-20260911-v1")
  const optionalUi = source.indexOf("./parts5/gear-mesh-ui-v1.js?v=parts-5-20260909-visual-v2")
  const runtimeReady = source.indexOf('window.__bricklabRuntimeReady = true')
  assert.ok(adapter >= 0)
  assert.ok(assistant > adapter, 'Smart Assembly starts only after the stable editor adapter is bound')
  assert.ok(assistant < performance, 'optional performance startup cannot gate Smart Assembly')
  assert.ok(assistant < optionalUi, 'later editor UI modules cannot gate Smart Assembly')
  assert.ok(assistant < runtimeReady, 'full runtimeReady cannot gate Smart Assembly')
  assert.match(source, /void startSmartAssembly\(\)/, 'Smart Assembly starts non-blocking')
  assert.doesNotMatch(source, /requestIdleCallback/, 'guidance activation must not wait for browser idle time')
