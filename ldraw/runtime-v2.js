...
async function cachedText(key, url) {
  ...
  const promise = fetchOk(url).then(response => response.text())
  textCache.set(key, promise)
  try { return await promise } catch (error) { textCache.delete(key); throw error }
}

export function fetchLDrawText(file) {
  const normalized = normalizeFile(file)
  return cachedText(`part:${normalized}`, rawUrl(PARTS_ROOT, normalized))
}

function fetchPrimitiveText(file) {
  const normalized = normalizeFile(file).replace(/^p\//i, '')
  return cachedText(`primitive:${normalized}`, rawUrl(PRIMITIVES_ROOT, normalized))
}
...
function convertedAxis(matrix, local = new THREE.Vector3(0, 1, 0)) {
  ...
  const value = local.clone().applyMatrix3(normal)
  value.set(value.x, -value.y, -value.z)
  return value.lengthSq() > 1e-8 ? value.normalize() : new THREE.Vector3(0, 1, 0)
}

function dedupePoints(items, tolerance = 0.10) {
  const result = []
  for (const item of items) {
    const existing = result.find(other => other.kind === item.kind && other.position.distanceTo(item.position) <= tolerance && Math.abs(other.axis.dot(item.axis)) > .78)
...
function pairOpenings(items, maxDistance = 1.25) {
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    for (let j = i + 1; j < items.length; j += 1) {
      ...
      if (Math.abs(items[i].axis.dot(items[j].axis)) < .55) continue
    ...
    if (best >= 0) {
      ...
      used.add(i)
      result.push({ ...items[i], position: items[i].position.clone(), axis: items[i].axis.clone() })
    }
  }
  return dedupePoints(result, .16)
}

async function primitiveInfo(file) {
  const basename = normalizeFile(file).split('/').pop() || ''
  if (primitiveInfoCache.has(basename)) return primitiveInfoCache.get(basename)
  const promise = (async () => {
    let description = ''
    try { description = parseHeader(await fetchPrimitiveText(basename), basename).description }
    catch { /* primitive name is still useful as a conservative fallback */ }
    const lower = description.toLowerCase()
    if (/^stud/i.test(basename)) {
      const tube = /tube|underside/i.test(lower) || /^(?:stud3|stud4|stud8|stud11)/i.test(basename)
      return { kind: tube ? 'tube' : 'stud', description }
    }
    if (/^(?:peghole|pinhol|pin-hole)/i.test(basename)) return { kind: 'pin-hole', description }
    if (/^axlehol(?:e|\d|[a-z]|-)/i.test(basename)) return { kind: 'axle-hole', description }
    return { kind: null, description }
  })()
  primitiveInfoCache.set(basename, promise)
  return promise
}

function featureFromPrimitive(kind, matrix) {
  if (kind === 'stud') {
    return { kind, position: convertedPoint(matrix), axis: convertedAxis(matrix, new THREE.Vector3(0, -1, 0)) }
  }
  if (kind === 'tube') {
    // Standard stud-tube primitives are open at local Y=-4. This maps to the
    // underside mating plane even when the part scales or flips the primitive.
...
function isSubpartReference(file) {
  const normalized = normalizeFile(file)
  return /^s\//i.test(normalized) || /^parts\/s\//i.test(normalized)
}

async function scanFeatures(file, text, parentMatrix, depth, stack, output) {
  for (const ref of type1References(text)) {
    const worldMatrix = parentMatrix.clone().multiply(ref.matrix)
    const info = await primitiveInfo(ref.file)
    if (info.kind) {
      output.push(featureFromPrimitive(info.kind, worldMatrix))
      continue
    }
    if (depth >= MAX_SUBPART_DEPTH || !isSubpartReference(ref.file)) continue
    const childFile = normalizeFile(ref.file).replace(/^parts\//i, '')
    if (stack.has(childFile)) continue
    try {
      const childText = await fetchLDrawText(childFile)
      const nextStack = new Set(stack)
      nextStack.add(childFile)
      await scanFeatures(childFile, childText, worldMatrix, depth + 1, nextStack, output)
    } catch (error) {
      console.debug?.(`[BrickLab LDraw] connector subpart skipped: ${childFile}`, error)
    }
  }
}
...
async function inferFeatures(file, text) {
  ...
  const promise = (async () => {
    const raw = []
    await scanFeatures(normalized, text, new THREE.Matrix4(), 0, new Set([normalized]), raw)
    const unique = dedupePoints(raw)
    return {
...
function connectorArray(raw, offset) {
  ...
  const add = (type, item, index) => {
    ...
    const axis = item.axis.clone()
    result.push({ id: `ldraw-${type}-${index}`, type, position: [point.x, point.y, point.z], axis: [axis.x, axis.y, axis.z] })
  }
  raw.studs.forEach((item, index) => add('stud', item, index))
  raw.tubes.forEach((item, index) => add('tube', item, index))
...
function inferMechanics(metadata, connectors) {
  ...
  if (spur && !/bevel|worm|rack|crown|clutch|differential|knob|turntable/.test(lower)) {
    ...
    if (Number.isFinite(teeth) && teeth >= 6 && teeth <= 80) mechanics.gear = { teeth, pitchRadius: teeth / 16, efficiency: .92 }
  }
  if (/^technic axle\s+\d/i.test(lower) && !/connector|hole|joiner|coupler/.test(lower)) mechanics.shaft = true
  if (mechanics.gear && !connectors.some(connector => connector.type === 'axle-hole')) delete mechanics.gear
...
async function loadPrototype(file) {
  ...
  const promise = (async () => {
    ...
    const mechanics = inferMechanics(metadata, connectors)
    return {
      model,
      metadata: { ...metadata, size: [size.x, size.y, size.z], connectorCount: connectors.length, subpartInference: true },
      connectors,
      mechanics,
    }
  })()
...
function fallbackSize(description = '') {
  ...
  const width = Math.max(.5, Number(match[1]) || 1)
  const depth = Math.max(.5, Number(match[2]) || 1)
  const height = Math.max(.4, Number(match[3]) || 1.2)
