const LOADER_ID = 'bricklab-project-loader-v1'
...
#${LOADER_ID}.is-leaving{opacity:0;pointer-events:none}
#${LOADER_ID} *{box-sizing:border-box}
.bl-load-top{position:absolute;top:27px;left:50%;width:min(430px,42vw);transform:translateX(-50%);display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center}
.bl-load-track{height:4px;border-radius:999px;background:rgba(255,255,255,.13);overflow:hidden;box-shadow:inset 0 0 9px rgba(0,0,0,.34)}
.bl-load-fill{display:block;width:0;height:100%;border-radius:inherit;background:#67f2cb;box-shadow:0 0 18px rgba(103,242,203,.35);transition:width .12s linear}
.bl-load-percent{min-width:34px;color:#7d8985;font-size:11px;font-variant-numeric:tabular-nums}
.bl-load-center{position:absolute;left:50%;top:48%;transform:translate(-50%,-50%);text-align:center}
.bl-load-logo{margin:0;text-transform:uppercase;font-size:clamp(32px,4vw,58px);font-weight:300;letter-spacing:.3em;text-indent:.3em}.bl-load-logo em{font-style:normal;font-weight:650;color:#67f2cb}
.bl-load-stage{margin-top:18px;color:#7b8783;font-size:9px;letter-spacing:.32em;text-transform:uppercase;white-space:nowrap}.bl-load-count{margin-top:9px;color:#515c58;font-size:9px;font-variant-numeric:tabular-nums}
.bl-load-orbit{position:absolute;left:50%;top:62%;width:260px;height:72px;transform:translate(-50%,-50%) rotateX(72deg);border:1px solid rgba(103,242,203,.12);border-radius:50%;box-shadow:0 0 42px rgba(103,242,203,.035)}
.bl-load-orbit:before,.bl-load-orbit:after{content:"";position:absolute;inset:16%;border:1px solid rgba(142,167,160,.1);border-radius:50%}.bl-load-orbit:after{inset:34%}
@media(max-width:760px){.bl-load-top{width:calc(100vw - 44px)}.bl-load-stage{white-space:normal;line-height:1.7}.bl-load-orbit{width:210px}}
@media(prefers-reduced-motion:reduce){#${LOADER_ID}{transition:none}.bl-load-fill{transition:none}}
`
...
async function consumeResponse(response, onFraction) {
  if (!response.ok) throw new Error(`HTTP ${response.status}`)
  const total = Number(response.headers.get('content-length')) || 0
  if (!response.body?.getReader) {
    const buffer = await response.arrayBuffer()
    onFraction(1)
    return { text: response.headers.get('content-type')?.includes('javascript') ? new TextDecoder().decode(buffer) : null }
  }
  ...
  while (true) {
    ...
    if (total) onFraction(Math.min(.98, received / total))
  }
  onFraction(1)
  const type = response.headers.get('content-type') || ''
  if (!type.includes('javascript') && !type.includes('text/')) return { text: null }
...
export function createProjectPreloader() {
  ...
  root.innerHTML = `
    <div class="bl-load-top"><div class="bl-load-track"><span class="bl-load-fill"></span></div><span class="bl-load-percent">0%</span></div>
    <div class="bl-load-center"><h1 class="bl-load-logo">BrickLab <em>3D</em></h1><div class="bl-load-stage">Подготовка проекта</div><div class="bl-load-count">0 файлов</div></div>
    <div class="bl-load-orbit" aria-hidden="true"></div>`
  ...
  const render = (fraction, label = null) => {
    const next = Math.max(shown, Math.min(.985, Number(fraction) || 0))
    shown = next
    fill.style.width = `${(next * 100).toFixed(1)}%`
    percent.textContent = `${Math.floor(next * 100)}%`
  ...
  const preload = async () => {
    ...
    const worker = async () => {
      while (cursor < queue.length) {
        ...
        const updateFraction = part => render((baseCompleted + Math.max(0, Math.min(1, part))) / Math.max(total, 1))
        try {
          const response = await fetch(url, { cache: 'force-cache', credentials: new URL(url).origin === location.origin ? 'same-origin' : 'omit' })
          const { text } = await consumeResponse(response, updateFraction)
          if (text && isJavaScript(url)) {
    ...
    const videoUrl = await firstExistingVideo()
    if (videoUrl && !seen.has(videoUrl)) {
      ...
      try {
        ...
        await consumeResponse(response, part => render((completed + part) / total))
      } catch (error) {
  ...
  const stageProgress = (value, label) => {
    const normalized = .94 + Math.max(0, Math.min(1, value)) * .05
    render(normalized, label)
  }
...
export const BrickLabProjectPreloader = Object.freeze({
  loaderId: LOADER_ID,
