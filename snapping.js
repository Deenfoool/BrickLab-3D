...
export function findSnapCandidate(selected, objects, maxDistance = 0.72) {
  ...
  for (const source of sourceDef.connectors) {
    ...
    for (const object of objects) {
      ...
      for (const target of targetDef.connectors) {
        ...
        if (score < bestScore) {
          bestScore = score
          best = { source, target, targetObject: object, targetWorld, distance }
        }
      }
    }
  }
...
export function applySnap(selected, candidate) {
  ...
  if (!selected.parent) return selected.position.add(worldDelta)
